...
exports.markAttendance = async (req, res) => {
  ...
  try {
    ...
    if (!attendance) {
      ...
      // For new attendance, check if any of the selected periods are already marked
      ...
      if (alreadyMarkedPeriods.length > 0) {
    ...
    for (const period of periods) {
      ...
      for (const student of students) {
        ...
        // Update the specific period's status
        studentAttendance.periods[period] = isAbsent ? 'absent' : 'present';
...
exports.getStudentAttendance = async (req, res) => {
  ...
  try {
    ...
    console.log('Fetching attendance between:', fiveDaysAgoStr, 'and', todayStr);
    ...
    console.log('Found recent attendance records:', recentAttendance.length);
    ...
    console.log('Sending response:', formattedAttendance);
    res.json(formattedAttendance);
  } catch (error) {
